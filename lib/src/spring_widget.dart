...
/// [SpringAnimatedTo] is a widget that animates a given [child] when its position changes for any reason.
...
///
/// Because [SpringAnimatedTo] need to be kept alive even if its position or depth in the widget tree is changed,
...
class _RenderAnimatedTo extends RenderProxyBox {
  ...
  /// for animation
  SpringSimulationController2D? _controller;
  ...
  void _startAnimation(Journey journey) {
    ...
    _controller!.value = (journey.from.dx, journey.from.dy);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (isScrolling) {
      ...
      if (isAnimating) {
        ...
        if (_journey.to != offset) {
          ...
          _journey = Journey(
            from: Offset(_controller!.value.x, _controller!.value.y) -
                Offset(
                  0,
                  (_scrollOffset ?? 0) -
    ...
    if (_controller != null) {
      ...
      final animationOffset =
          _controller!.value.toOffset() + Offset(0, scrollGap);
